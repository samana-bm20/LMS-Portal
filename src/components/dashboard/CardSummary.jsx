...
const Card2 = ({ color, title, total, active, dead, owner, alt, pid }) => {
    const navigate = useNavigate();
    const ownerIcon = ownerIconMap[owner] || newLogo;

    const handleProductClick = (title) => {
        console.log('product card clicked')
        navigate(`/lms/leads?product=${encodeURIComponent(pid)}`)
    }
    
    return (
        <div
            className={`${color} text-white shadow-md rounded-lg p-4 m-2 cursor-pointer`}
